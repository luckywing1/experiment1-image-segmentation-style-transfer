...
- **任务B**：基于 AdaIN 的实时图像风格迁移，支持风格强度调节、摄像头实时迁移、批量处理、**掩码分区风格迁移**。
...
在任务B 主界面点击「🎨 掩码分区风格」按钮打开子窗口，可对同一张图片的**不同区域应用不同风格**。

**操作步骤：**

1. 点击「加载内容图」选择要处理的图片
2. 点击「**+ 添加区域**」新建一个掩码区域（每个区域会分配不同颜色标识）
3. 在左侧画布上用**画笔涂抹**，标记该区域的范围（彩色半透明叠加显示）
4. 点击「**选择风格图**」为当前区域绑定一张风格参考图，并调节「强度」滑块
5. 重复步骤 2–4，继续添加其他区域（不同区域可使用完全不同的风格）
6. 点击「**▶ 执行掩码风格迁移**」，各区域分别风格化后按掩码合成
7. 点击「保存结果」保存

> **提示**：
>
> - 未被任何掩码覆盖的区域自动保留原图不变
> - 画笔支持「橡皮擦模式」修改掩码，或「清除当前区域掩码」重新绘制
> - 掩码窗口共享主界面已加载的模型，无需重复等待加载
